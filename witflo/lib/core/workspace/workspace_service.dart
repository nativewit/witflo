...
import 'package:witflo_app/core/workspace/workspace_service_interface.dart';
...
class WorkspaceService implements IWorkspaceService {
  ...
  /// Creates the content for the workspace marker file (.witflo-workspace).
  String _createWorkspaceMarkerContent() {
    final now = DateTime.now().toUtc().toIso8601String();
    return '''# Witflo Workspace
# This directory contains encrypted vaults managed by Witflo.
# Created: $now
# 
# Structure:
...
''';
  ...
  /// Validates that a directory is a valid Witflo workspace.
  ///
  /// Checks for:
  ...
  Future<WorkspaceConfig> switchWorkspace(String newRootPath) async {
    ...
    if (!await isValidWorkspace(newRootPath)) {
      throw WorkspaceException('Not a valid Witflo workspace: $newRootPath');
    }
